...
func TestCartServiceHandler(t *testing.T) {
	...
	handler := NewHandler(productStore, orderStore, nil, db)
...
func (m *mockOrderStore) CreateOrder(tx *sql.Tx, order types.Order) (int, error) {
	return 0, nil
}

func (m *mockOrderStore) CreateOrderItem(tx *sql.Tx, orderItem types.OrderItem) error {
	return nil
}
